...
const width = 800;
const height = 800;
const app_name = "1_Textured_Cubes";
...
pub fn main(init: std.process.Init) !void {
    ...
    const dirt_img_path = try path_resolver.resolvePath(gpa, .Assets, "minecraft_dirt.png");
    defer gpa.free(dirt_img_path);

    var material = try rdr.msh.Material.createFromFile(
        &renderer,
        dirt_img_path,
        .R8G8B8A8_Unorm,
    );
    defer material.deinit(&renderer);

    var cube: rdr.msh.Mesh = undefined;
    try cube.init(&renderer, &rdr.msh.cube_vertices, &rdr.msh.cube_indices, &material);
