...
pub const Mesh = struct {
    ...
    pub fn init(self: *@This(), gpu_device: *dev.GpuDevice, vertices: []const Vertex, indices: []const u32) !void {
        ...
        self.vertex_buffer = try buf.Buffer.init(gpu_device, .Vertex, vertex_buf_size);
        ...
        self.index_buffer = try buf.Buffer.init(gpu_device, .Index, index_buf_size);
    ...
    pub fn deinit(self: *@This(), gpu_device: *dev.GpuDevice) void {
        self.index_buffer.deinit(gpu_device);
        self.vertex_buffer.deinit(gpu_device);
    }
